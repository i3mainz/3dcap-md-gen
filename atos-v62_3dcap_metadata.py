...
#atos v6.2
referencepointid="r_id"
globalreferencepointid="r_id"
...
while prj < 1:
	...
	while p < len( gom.app.projects):		
		...
		while m < len(gom.app.projects[p].measurements):
			...
			#  @param measurementclass
			#  @param application indicates if the parameter is a software parameter or an external parameter
			def measurements_sensor (beschreibung, description, keyword, einheit, uri, measurementclass, application):			
				dir = {}
				dir.clear()
				# zeitangaben
	...
	#		infos_measurements (beschreibung, description, keyword, einheit, uri, measurementclass, application)
			...
			# Breite des Messvolumens
			beschreibung = "Breite des Messvolumens"
			description = "Measurement volume width"
			...
			measurementclass = None
			application = "true"
			cal_measurements (beschreibung, description, keyword, einheit, uri, measurementclass, application)
			...
			if len(dic_measurement_cal) > 0:
				dic_measurement["calibration"] = dic_measurement_cal
			if len(dic_measurement_sensor) > 0:
				dic_measurement["capturing_device"] = dic_measurement_sensor
	...
	while me < len( gom.app.meshes):
		...
		def infos_meshes (beschreibung, description, keyword, einheit, uri, measurementclass, application):
			dir = {}
			dir.clear()
			if keyword == 'comment':
